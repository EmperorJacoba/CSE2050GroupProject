# CSE 2050 Group Project: Milestone 2
## by Justin Elak and Jacob Russell

All python files are contained in /pyfiles. To run the main program (parse and display student/course data), run the script main.py
in /pyfiles with four command line arguments. 1: course catalog, 2: student data, 3: university data (enrollments_CSE10), 4: course catalog with capacity, 5: prerequisites.

> The requested/recommended demo_milestone3.py file is implemented via main.py

CSV file format is expected in the same format as outlined in the PDF specifications document. 
Given CSV files are in pyfiles/csv_files for simplicity when testing python scripts.

### Sample command in /pyfiles (windows):

`python main.py .\csv_files\course_catalog.csv .\csv_files\university_data.csv .\csv_files\enrollments_CSE10.csv .\csv_files\course_catalog_CSE10_with_capacity.csv .\csv_files\cse_prerequisites.csv`

### Test cases

To run testcases, run pyfiles/milestone3_testcases.py.